...
export function renderCommands(
  ...
  for (const discoveredPackage of packages) {
    ...
    lines.push(
      `    provider: ${discoveredPackage.metadata.provider ?? "none"}`,
    );
    ...
    const loadedProvider = providerRegistry
      ...
      .find(
        (provider) =>
          provider.discoveredPackage.packageName ===
...
export function renderProviderLoadFailure(error: unknown): string {
  return [
    `Ankh provider manifest loading failed unexpectedly: ${getErrorMessage(error)}`,
    "",
  ].join("\n");
}
...
export function renderUnknownCategory(category: string): string {
  return [
    `Unknown Ankh category: ${category}`,
    "Try:",
    "  ankh commands",
    "",
  ].join("\n");
...
function renderDiagnostics(
  ...
  for (const diagnostic of diagnostics) {
    const scopeParts = [
      ...
      "providerModulePath" in diagnostic
