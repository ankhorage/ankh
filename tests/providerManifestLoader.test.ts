...
describe('loadProviderManifests', () => {
  it('loads valid default-exported provider manifests via file URLs', async () => {
    ...
    expect(result.providers[0]?.providerModuleDefaultExport).toEqual(validManifest);
...
async function writeProviderModule(packageRoot: string, content: string): Promise<void> {
