...
describe('createProviderRegistry', () => {
  ...
  it('lists loaded providers and command descriptors separately from package metadata', () => {
    ...
    expect(registry.findByCategory('infra')?.manifest.id).toBe('@ankhorage/infra');
    expect(registry.listCommands()).toEqual([
      {
        aliases: ['start'],
        capability: 'infra.up',
  ...
  it('resolves canonical paths, aliases, and longest path matches', () => {
    ...
    expect(registry.resolveCommand('infra', ['start', '--watch'])?.argv).toEqual(['--watch']);
    expect(registry.resolveCommand('infra', ['port', 'forward', 'db'])?.command.path).toEqual([
      'port',
      'forward',
    ]);
